...
outdir = outdir + "model_cosmo_" + sys.argv[1] + "/" # directory labelled by model number
...
theta_fid = tf.constant([0.3, -1.0], dtype=tf.float32) #  0.13, 2.56, -19.3, 0.1, 0.0, 1.0, 0.0, 0.1
...
for i in range(10):
    ...
    theta.append(np.load(datadir + 'data/theta_cosmo_%d.npy'%(i)))

theta = np.concatenate(theta)[:, :n_theta]

# z, mb, dmb^2, x1, dx1^2, c, dc^2, mb/dmb^2, x1/dx1^2, c/dc^2
data = np.load(datadir + 'data/data_cosmo_obs_full_100k.npy')
...
Model.train((data, theta, score_mask, fisher_mask), lr=1e-6, epochs=100)
model_checkpoint(Model=Model, checkpoint=3)
Model.train((data, theta, score_mask, fisher_mask), lr=5e-7, epochs=100)
