...
### my version of the distance modulus
...
def hubble(z,omegam, omegade,w):
    ...
    I = scipy.integrate.quad(integrand, 0., z,args = (omegam,omegade,w))[0]
    return I
...
def generate_sn1a(theta):
    ...
    #         elif(distribution_on_sky=="uniform"):
    ...
    mu_i = muz(omegam, w, z, z) #omegam, w, z, z_helio#distance_modulus(z,true_omegam,true_omegade,l,b)

    #Step 3, Draw latent x1_i, c_i, M_i
    M_i = np.random.normal(loc=M_0, scale = sigma_res)
...
def make_plot(inputs):
  sim_z, sim_zhel, sim_mb, sim_dmb, sim_x1, \
     sim_dx1, sim_c, sim_dc, sim_l, sim_b, sim_true_mb, sim_true_x1, sim_true_c = inputs
...
def simulator(theta, number_of_sne=250, save_data=True, plot=False):
  ...
  if save_data:
    ...
    np.savetxt(lc_params_filename, simulated_data, delimiter=' ',header = headers,comments="")
    np.savetxt(covariance_filename, simulated_covariance, delimiter=' ')

  if plot:
    inputs =  sim_z, sim_zhel, sim_mb, sim_dmb, sim_x1, sim_dx1, sim_c, sim_dc, sim_l, sim_b, sim_true_mb, sim_true_x1, sim_true_c
    make_plot(inputs)

  return simulated_data, simulated_covariance
