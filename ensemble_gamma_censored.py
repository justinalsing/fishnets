...
outdir = outdir + "model_censored_" + sys.argv[1] + "/" # directory labelled by model number
...
# load theta and data
theta = np.load(datadir + 'theta_gamma_censored.npy')
data = np.load(datadir + 'data_gamma_censored.npy')
...
Model.train((data, theta, score_mask, fisher_mask), lr=1e-3, epochs=20)
Model.train((data, theta, score_mask, fisher_mask), lr=5e-4, epochs=200)
model_checkpoint(Model=Model, checkpoint=0)
Model.train((data, theta, score_mask, fisher_mask), lr=1e-4, epochs=200)
model_checkpoint(Model=Model, checkpoint=1)
Model.train((data, theta, score_mask, fisher_mask), lr=5e-5, epochs=200)
...
data_test = np.load(datadir + 'data_censored_test_ndata_10k.npy')
theta_test = np.load(datadir + 'theta_censored_test_ndata_10k.npy')
# make data neural-net friendly
datamax = 500.
tmax = 10.
...
plt.legend(frameon=False)
plt.savefig(outdir + 'mean_test_%d.png'%(int(sys.argv[1])))
plt.savefig(plotdir + 'mean_test_%d.png'%(int(sys.argv[1])))
plt.close()

plt.hist(mle[:,1].numpy() - theta[:,1].numpy(), bins = 60, histtype='step', density=True, label='learned score MLE')
plt.xlabel('mle_scale - scale')
plt.legend(frameon=False)
plt.savefig(outdir + 'scale_test_%d.png'%(int(sys.argv[1])))
plt.savefig(plotdir + 'scale_test_%d.png'%(int(sys.argv[1])))
plt.close()